    ...
    for i in range(32, 0, -1):
        ...
        for state in states:
            ...
            if can_make_geode_bot(reserve_array, blueprint_array):
                ...
                geo_bot_made = True
            if not geo_bot_made and can_make_obsidian_bot(reserve_array, blueprint_array):
                new_reserve_array = reserve_array.copy()
                new_bot_array = bot_array.copy()
        ...
        states = round_states
        states = reduce_states(states, 24 - i)
    return blueprint_array[0] * np.max([state[0][3] for state in states])


total = 0
for blueprint in blueprints:
    print(total)
    total += get_blueprint_quality_level(blueprint, reserves.copy(), bots.copy())
print(total)
