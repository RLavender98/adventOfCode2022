...
def find_shortest_path_through(valley_starting_map):
    ...
    while not found_exit:
        ...
        if any([pos == (len(valley_starting_map) - 2, len(valley_starting_map[0]) - 2) for pos in pos_array]):
            time += 1
            found_exit = True
    pos_array = [(len(valley_starting_map) - 1, len(valley_starting_map[0]) - 2)]
    found_entry = False
    while not found_entry:
        ...
        for pos in pos_array:
            ...
            for step in steps_available:
                new_pos_array.append(step)
        pos_array = remove_duplicate_positions(new_pos_array)
        pos_array.append((len(valley_starting_map) - 1, len(valley_starting_map[0]) - 2))
        if any([pos == (1, 1) for pos in pos_array]):
            time += 1
            found_entry = True
    pos_array = [(0, 1)]
    found_exit = False
...
find_shortest_path_through(valley)
